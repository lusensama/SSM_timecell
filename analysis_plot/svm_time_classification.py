...
def extract_time_slices(activity, use_middle_only=True, middle_size=5):
    ...
    """
    time_ranges = [
        (5, 24),    # Class 1: 20 bins
        (38, 57),   # Class 2: 20 bins
        (71, 90),   # Class 3: 20 bins
        (105, 124)  # Class 4: 20 bins
    ]
    ...
    for start, end in time_ranges:
        total_bins = end - start + 1  # 20
        
        if use_middle_only:
            # Compute the start/end of the middle portion
...
def prepare_dataset(slices_list, test_size=4500, random_seed=42):
    ...
    # Randomly select training and test episodes
    ...
    for class_idx, time_slice in enumerate(slices_list):
        # time_slice: (5000, 20, 80)
        
        # Training data
        train_slice = time_slice[train_indices]  # (500, 20, 80)
        # Flatten: (500, 20*80)
        ...
        # Test data
        test_slice = time_slice[test_indices]  # (4500, 20, 80)
    ...
    # Combine all classes
    X_train = np.vstack(X_train_list)  # (500*4, 20*80) = (2000, 1600)
    X_test = np.vstack(X_test_list)    # (4500*4, 20*80) = (18000, 1600)
    y_train = np.concatenate(y_train_list)  # (2000,)
    y_test = np.concatenate(y_test_list)    # (18000,)
    
    # Compute actual number of time bins
...
def main():
    ...
    test_size = 4500  # Test set size (4500 test, 500 train: 10%/90%)
    random_seed = 42
    ...
    # 2. Extract time slices (use middle 5 time steps)
