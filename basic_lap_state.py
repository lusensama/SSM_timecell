...
def sort_resp(total_resp, norm=True):
    ...
    normalized_matrix = np.zeros_like(unsorted_matrix)
...
def sort_sub(resp, laps=4, t=30):
    ...
    for i in range(laps):
        seg = resp[:, i*t:(i+1)*t, :]
        sorted_indices, sorted_real_matrix, _ = sort_freq_resp(seg, norm=True)

        unsorted_mat_raw = reshape_resp(seg)

        sorted_mat_raw = unsorted_mat_raw[sorted_indices]
        unsorted_mat = normalize_activity(unsorted_mat_raw)

        sorted_segments.append(sorted_real_matrix)
        sorted_mat_raw_segments.append(sorted_mat_raw)
        unsorted_mat_segments.append(unsorted_mat)
        unsorted_mat_raw_segments.append(unsorted_mat_raw)

    sorted_real_matrix = np.concatenate(sorted_segments, axis=1)
    sorted_mat_raw = np.concatenate(sorted_mat_raw_segments, axis=1)
    unsorted_mat = np.concatenate(unsorted_mat_segments, axis=1)
