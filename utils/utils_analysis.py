...
def sort_freq_resp(total_resp, norm=True):
    ...
    # Step 2: Count peaks in the time-domain signal (include endpoints)
    def _count_peaks(sig, prom_frac=0.2):
        ...
        for j in range(1, len(sig) - 1):
            if sig[j] >= thr and sig[j] >= sig[j - 1] and sig[j] > sig[j + 1]:
                peaks += 1
        # right endpoint (avoid double count when len==2)
    ...
    # Step 3: Sort by peak count, then by timing of max response for neurons with 1 OR 2 peaks
    # Get the timing of maximum response for each neuron
    max_indices = np.argmax(normalized_matrix, axis=1)
    
    # Create a composite sorting key: (peak_count, max_index)
    # For neurons with 1 or 2 peaks, combine them into one group (use 1 as group ID) and sort by timing
    # For others, use original index to maintain order within their peak count group
    sorting_keys = []
    for i in range(n_neurons):
        if peak_counts[i] == 1 or peak_counts[i] == 2:
            # Treat 1 and 2 peaks as the same group, sort together by timing
            sorting_keys.append((1, max_indices[i]))
        else:
            # Use original index to maintain relative order for non-1/2 peak neurons
    ...
    # print("Peak counts in sorted order:", sorted_peak_counts)
