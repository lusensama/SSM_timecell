...
def plot_lap_choices(net, env, n_total_episodes=1000, spike=False, layer2=False, save_path='./figures'):
    ...
    for i_episode in tqdm(range(n_total_episodes)):
        ...
        while not done:
            ...
            observation = new_obs
    avg_laps = np.mean(all_laps, axis=0)
    plt.clf()
...
def train_laps(
    ...
    eval_every: int,
    n_eval_episodes: int,
    vary_lap_len: bool = False,
    lap_len_range: tuple = (20, 45),
    pause_range: tuple = (0, 15),
):
    env = Laps_Counting(
        final_rwd=10,
        seed=seed,
        ...
        vary_lap_len=vary_lap_len,
        lap_len_range=lap_len_range,
...
@torch.no_grad()
def collect_hidden_laps(net, env, device, n_episodes: int, lap_length: int, lap_count: int, spike: bool = False, layer2: bool = False, max_T: int = None, return_meta: bool = False):
    """
    ...
    max_T : int, optional
        Override the time-axis size of the collected arrays. Needed when
        env.vary_lap_len=True, since episode length is no longer a fixed
        function of lap_length/lap_count. Defaults to the old fixed-length
        sizing when None.
    return_meta : bool, optional
        If True, also return a dict of per-episode lap timing metadata
        (lap_start_times, lap_mid_times, lap_durations, lap_ends,
        episode_lengths), needed to decouple lap count from absolute time
        in downstream decoding analyses. Default False preserves the
        original 4-value return for existing callers.
    ...
    meta : dict, only if return_meta=True
        Per-episode lap timing metadata, arrays of shape (n_episodes, lap_count)
        (or (n_episodes,) for episode_lengths).
    """
    total_T = max_T if max_T is not None else lap_length * lap_count + lap_count - 1 + 10
    ...
    if return_meta:
        lap_start_times = np.full((n_episodes, lap_count), -1, dtype=np.int64)
        lap_mid_times = np.full((n_episodes, lap_count), -1, dtype=np.int64)
        lap_durations = np.full((n_episodes, lap_count), -1, dtype=np.int64)
        lap_end_times = np.full((n_episodes, lap_count), -1, dtype=np.int64)
    ...
    for i_episode in tqdm(range(n_episodes)):
        ...
        if return_meta:
            n_meta = min(lap_count, len(env.lap_ends))
            lap_start_times[i_episode, :n_meta] = env.lap_start_times[:n_meta]
            lap_mid_times[i_episode, :n_meta] = env.lap_mid_times[:n_meta]
            lap_durations[i_episode, :n_meta] = env.lap_durations[:n_meta]
            lap_end_times[i_episode, :n_meta] = list(env.lap_ends)[:n_meta]
        ...
        if return_meta:
            episode_lengths[i_episode] = env.elapsed_t

    if return_meta:
        meta = {
            'lap_start_times': lap_start_times,
            'lap_mid_times': lap_mid_times,
            'lap_durations': lap_durations,
            'lap_end_times': lap_end_times,
            'episode_lengths': episode_lengths,
        }
        return full_resp1, full_resp2 if layer2 else None, spiking_entries1, spiking_entries2 if layer2 else None, meta
...
def evaluate_and_plot_model(
    ...
    with open(results_file, 'w') as f:
        ...
        f.write(f"Count-matching accuracy: {eval_acc:.6f}%\n")
        f.write(f"Mean VP (timing) score: {mean_vp:.6f}\n")
        ...
        f.write(f"  Vary lap len: {vary_lap_len}\n")
        f.write(f"  Lap len range: {lap_len_range}\n")
        f.write(f"  Pause range: {pause_range}\n")
        f.write(f"  Seed: {seed}\n")

    results_json = os.path.join(save_dir, "laps_evaluation_results.json")
    with open(results_json, 'w') as f:
        json.dump({
            "model_path": model_path,
            "n_eval_episodes": n_eval_episodes,
            "count_matching_accuracy_pct": float(eval_acc),
            "mean_vp_score": float(mean_vp),
            "n_neurons": n_neurons, "lap_length": lap_length, "lap_count": lap_count,
            "spike": spike, "layer2": layer2, "approx": approx,
            "vary_lap_len": vary_lap_len, "lap_len_range": list(lap_len_range),
            "pause_range": list(pause_range), "seed": seed,
        }, f, indent=2)

    print(f"Results saved to: {results_file}")

    print("Collecting hidden states for analysis...")
    max_T = lap_count * (lap_len_range[1] + pause_range[1]) + lap_count + 20 if vary_lap_len else None
    full_resp1, _, _, _, meta = collect_hidden_laps(
        net=net,
        env=eval_env,
        device=device,
    ...
    meta_path = os.path.join(save_dir, f'./lap_counting_{seed}_meta.npz')
    np.savez_compressed(meta_path, **meta)
    print(f"Per-episode lap timing metadata saved to {meta_path}")
    if not vary_lap_len:
...
def main():
    ...
    parser.add_argument("--vary_lap_len", action='store_true', default=False,
                       help="Draw an independent duration (and optional pause) per lap instead of a fixed lap_length, to decouple lap count from absolute elapsed time (Experiment 2 de-confounding).")
    parser.add_argument("--lap_len_range", type=int, nargs=2, default=(20, 45), metavar=('MIN', 'MAX'),
                       help="Per-lap duration range (steps) when --vary_lap_len is set.")
    parser.add_argument("--pause_range", type=int, nargs=2, default=(0, 15), metavar=('MIN', 'MAX'),
                       help="Post-landmark dwell-pause range (steps) when --vary_lap_len is set.")
    ...
    if args.mode == 'eval':
        eval_acc, save_dir = evaluate_and_plot_model(
            ...
            vary_lap_len=args.vary_lap_len,
            lap_len_range=tuple(args.lap_len_range),
    ...
    if args.vary_lap_len:
        save_dir_str += f'_varylap{args.lap_len_range[0]}-{args.lap_len_range[1]}_pause{args.pause_range[0]}-{args.pause_range[1]}'
    else:
    ...
    max_T = args.lap_count * (args.lap_len_range[1] + args.pause_range[1]) + args.lap_count + 20 if args.vary_lap_len else None
    full_resp1, _, _, _, meta = collect_hidden_laps(
        net=net,
        env=env,
        device=device,
    ...
    np.save(os.path.join(save_dir, f'./lap_counting_{args.seed}_activity.npy'), full_resp1)
    meta_path = os.path.join(save_dir, f'./lap_counting_{args.seed}_meta.npz')
    np.savez_compressed(meta_path, **meta)
    print(f"Per-episode lap timing metadata saved to {meta_path}")
    if not args.vary_lap_len:
